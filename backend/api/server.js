...
try {
  ...
  // console.warn('Failed to parse FIREBASE_SERVICE_ACCOUNT_KEY from env, falling back to file');
...
const allowedOrigins = ['http://localhost:3000', 'http://127.0.0.1:3000', 'https://insync-eight.vercel.app', 'https://in-sync-iota.vercel.app'];
...
// app.listen(8888); // Removed duplicate listen
...
app.get('/api/spotifylogin', function (req, res) {
  ...
  res.cookie(stateKey, state, { httpOnly: false, secure: cookieSecure, sameSite: cookieSameSite });
      ...
      if (!error && response.statusCode === 200) {
        ...
        request.get(options, async function (error, response, body) {
          ...
          try {
            ...
            res.cookie('user_id', user._id.toString(), { httpOnly: true, secure: cookieSecure, sameSite: cookieSameSite });
          } catch (error) {
          ...
          res.cookie('firebase_token', firebaseToken, { httpOnly: false, secure: cookieSecure, sameSite: cookieSameSite });
          ...
          request.get(topTracksOptions, async function (error, response, topTracksBody) {
            ...
            request.get(savedTracksOptions, async function (error, response, savedTracksBody) {
              ...
              res.cookie('spotify_access_token', access_token, { httpOnly: true, secure: cookieSecure, sameSite: cookieSameSite });
              res.cookie('spotify_refresh_token', refresh_token, { httpOnly: true, secure: cookieSecure, sameSite: cookieSameSite });
...
app.post('/api/logout', (req, res) => {
  res.clearCookie('spotify_access_token', { httpOnly: true, secure: cookieSecure, sameSite: cookieSameSite });
  res.clearCookie('spotify_refresh_token', { httpOnly: true, secure: cookieSecure, sameSite: cookieSameSite });
  res.clearCookie('firebase_token', { httpOnly: false, secure: cookieSecure, sameSite: cookieSameSite });
  res.clearCookie('user_id', { httpOnly: true, secure: cookieSecure, sameSite: cookieSameSite });
...
app.get('/api/spotify-user-data', (req, res) => {
  ...
  // logger.info(`Cookies received: ${JSON.stringify(req.cookies)}`); // Debug logs
  if (!accessToken) {
    logger.error('Spotify access token not found in cookies');
...
app.get('/api/spotify-top-artists', (req, res) => {
  const accessToken = req.cookies.spotify_access_token;
  if (!accessToken) {
    logger.error('Spotify access token not found for top artists');
    return res.status(401).json({ error: 'Spotify access token not found' });
  }
